...
call "%PREFIX%\python.exe" -c "import shapefile" && EXIT /B 0 || ECHO "Installing pyshp to fix shapefile module"
call "%PREFIX%\Scripts\conda.exe" install -y "%PREFIX%\pkgs\pyshp-PYSHPVERSION.tar.bz2" -n root || ECHO "Could not install pyshp. Run 'conda install -c conda-forge pyshp'"
